...
public class EbookServiceImpl extends ServiceImpl<EbookMapper, Ebook> implements IEbookService {
    ...
    @Override
    public List<Ebook> listByname(EbookQueryReq req) {
    ...
    @Override
    public PageResp<Ebook> listByPage(EbookQueryReq req) {
        QueryWrapper<Ebook> queryWrapper = new QueryWrapper<Ebook>();
        //第一个参数：该参数是一个布尔类型，只有该参数是true时，才将like条件拼接到sql中；本例中，如果name字段不为空，则拼接name字段的like查询条件；
        queryWrapper.like(StringUtils.isNotBlank(req.getName()),"name",req.getName());
        queryWrapper.eq(ObjectUtils.isNotEmpty(req.getCategoryId2()),"category2_id",req
                .getCategoryId2());
        //创建分页对象
        Page<Ebook> page = new Page<>(req.getPage(),req.getSize());
        page = this.baseMapper.selectPage(page, queryWrapper);
        List<Ebook> resps = CopyUtil.copyList(page.getRecords(), Ebook.class);
        //创建返回对象
    ...
    @Override
    public void saveBook(Ebook req) {
        if (ObjectUtils.isEmpty(req.getId())) {// 新增
