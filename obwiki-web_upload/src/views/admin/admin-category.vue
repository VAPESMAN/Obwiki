<template>
    <div>
        ...
        <a-modal
title="电子书表单" v-model:visible="modalVisible" :confirm-loading="modalLoading" @ok="handleModalOk">
            <a-form :model="category" :label-col="{ span: 6 }" :wrapper-col="{
                span: 18
            }">
                ...
                <a-form-item label="父分类">
                    <a-select ref="select" v-model:value="category.parent" style="width: 120px">
                        ...
                        <a-select-option v-for="c in level1" :key="c.id" :value="c.id"
                            :disabled="category.id === c.id">{{ c.name }}</a-select-option>
                    </a-select>
                </a-form-item>
