...
const handleQuery = () => {
  ...
  axios.get("/doc/all/" + route.query.ebookId).then((response) => {
    ...
    if (data.success) {
      ...
      level1.value = [];
      level1.value = Tool.array2Tree(toRaw(docs.value), 0);
      ...
      console.log("接口 /doc/all 返回 data:", data);
      console.log("docs.value:", docs.value);
      level1.value = Tool.array2Tree(toRaw(docs.value), 0);
      console.log("level1.value:", level1.value);
      console.log("初始化 doc.value:", doc.value);
...
const handleQueryContent = (id: number) => {
  axios.get("/content/findContent/" + id).then((response) => {
    ...
    if (data.success) {
      ...
      console.log(`/content/findContent/${id} 返回 data:`, data);
...
const onSelect = (selectedKeys: any, info: any) => {
  ...
  if (Tool.isNotEmpty(selectedKeys)) {
    //选中某一个节点时，加载该节点的文档信息
    doc.value = info.selectedNodes[0].dataRef;
    // 加载内容
    console.log('选中节点 selectedKeys:', selectedKeys, 'info:', info);
    console.log("选中文档 doc.value:", doc.value);
